...
\begin{frame}{Challenges in Covariance Estimation}
    \begin{enumerate}
        ...
        \item Dimensionality of covariance matrix
...
\begin{frame}{Backup 1: Bootstrap}
    Efron (1979)

    RV $X$

    $n$ iid realizations - $x_1, \cdots x_n$

    distribution parameter to be estimated - $\phi$ e.g. sample mean.

    estimator - $\hat{\phi}$

    1 replication - calculated from random sample of size $n$ with replacament.

    Bootstrap distribution is generated by considering all possible bootstrap replications of $\hat{\phi}$
\end{frame}

\begin{frame}{Backup 2: Bayesian Bootstrap}

    Rubin (1981): Let $f_i = 0, 1/n, \cdots, 1$ - proportion of $x_i$ draws in 1 replication

    $\implies$ Bootstrap sample mean = $\sum_{i=1}^{n}f_i x_i$

    Assumption: Each replication drawn from sample CDF, which equals population CDF. In Bayesian BB: Values of $X$ not observed have zero posterior probability

    \begin{enumerate}
        \item Draw $(n-1)$ ordered uniform variates $u_1, \cdots u_{n-1}$
        
        \item Gaps $g_i = u_i - u_{i-1}$
        
        \item Use $g_i$ as the proportion of draws for each $x_i$ and calculate $\hat{\phi} = \sum_{i=1}^{n}g_i x_i$
    \end{enumerate}

    \textbf{Interpretation}: BB simulates posterior distribution of $\phi$ directly instead of frequency statements about estimates of $\phi$.
