...
Commonly, forward UQ addresses propagation of uncertainties from $\theta$ to $\hat{u}$

\textbf{Neglected: } \emph{Verification error} - Discretization uncertainty is often ignored in practice. Introduces perturbations leading to exponential divergence from exact solution. \textcolor{red}{(discuss details)}
...
\begin{enumerate}
    ...
    \item Probabilistic Numerics \cite{hennig_probabilistic_2015} - highlight connections between `deterministic' numerical algorithms and probabilistic priors, Bayesian quadrature methods.
    
    \item GP Learning for noisy state data \cite{ye_gaussian_2023} - likelihood using correlation between state data and derivatives (discuss later!)
\end{enumerate}
...
\textbf{Posterior Model Inference: }
